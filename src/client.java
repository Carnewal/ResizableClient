...
//	notifyItemSpawn(item, i + baseX, j + baseY);
	...
	public void drawTabBack() {
		...
		if(tabInterfaceIDs[tabID] != -1 && drawLongTabs && is554)
 ...
 //	   myPassword = "";
	...
	public void processMapAreaClick() {
		...
		if (!newMap) {
			if(super.mouseX >= 742+x29 && super.mouseX <= 764+x29 && super.mouseY >= 1+y2 && super.mouseY <= 23+y2) {
				logIconHPos = 1;
			} else {
				logIconHPos = 0;
			}
	...
	private void loadingStages() {
		if(lowMem && loadingStage == 2 && ObjectManager.anInt131 != plane) {
			...
			aRSImageProducer_1165.drawGraphics(isFullScreen ? 0 : 4, super.graphics, isFullScreen ? 0 : 4);
			loadingStage = 1;
	...
	private void mainGameProcessor()
	{
		...
		{
		if (clientWidth-765 != extraWidth || clientHeight-503 != extraHeight || (isFullScreen && (Jframe.frame.getWidth()-11 != clientWidth || Jframe.frame.getHeight()-58 != clientHeight))) 
			{
				...
				if (isFullScreen) {
					...
					try {
						...
						//Jframe.frame.resize(new Dimension(765+extraWidth, 503+extraHeight+46));
					} catch(Exception e) {
						e.printStackTrace();
					}
				} else {
			...
			for(int i8 = 0; i8 < 9; i8++)
			{
				...
				ai[i8] = l8 * i9 >> 16;
					...
					try {
						...
						Jframe.frame.resize(new Dimension(765+16, 503+46+14));
					} catch(Exception e) {
						e.printStackTrace();
					}
				}
  ...
  */
            ...
            if (flag8) {
				...
				case 40004:
					if(isFullScreen) {
						...
						extraWidth = 50;
						hasBeenBlanked = false;
						//Jframe.frame.resize(new Dimension(765+16, 503+46+15));
					} else {
						isFullScreen = true;
						extraWidth = 300;
						extraHeight = 100;
	...
	private void processTabClick() {
		...
		if(super.clickMode3 == 1) {
			...
			if(super.saveClickX >= 742+x && super.saveClickX <= 764+x && super.saveClickY >= 1+y && super.saveClickY < 24+y && tabInterfaceIDs[10] != -1) 
			{
                needDrawTabArea = true;
                tabID = 10;
                tabAreaAltered = true;
		...
		if(super.clickMode3 == 1)
		{
			...
			if(super.saveClickX >= 742+x && super.saveClickX <= 764+x && super.saveClickY >= 1+y+y2 && super.saveClickY < 24+y+y2 && tabInterfaceIDs[10] != -1)
            {
				needDrawTabArea = true;
				tabID = 10;
				tabAreaAltered = true;
			...
			if(super.saveClickX >= 520 && super.saveClickX <= 561 && super.saveClickY >= 466 && super.saveClickY < 502 && tabInterfaceIDs[14] != -1)
			{
				needDrawTabArea = true;
				tabID = 14;
				tabAreaAltered = true;
	...
	public void processRightClick() {
		...
		int y = isFullScreen ? extraHeight-40 : 0;
		   ...
		   if(super.mouseX > 560+extraWidth && super.mouseY > 165+extraHeight && super.mouseX < 752+35+extraWidth && super.mouseY < 426+35+extraHeight && isFullScreen && drawLongTabs && !newMap) {
				if(invOverlayInterfaceID != -1 && isFullScreen && !newMap) {
				...
				} else if(tabInterfaceIDs[tabID] != -1 && isFullScreen && !newMap) {
		   ...
		   if(super.mouseX > 560+extraWidth && super.mouseY > 165+extraHeight && super.mouseX < 752+35+extraWidth && super.mouseY < 426+35-6+extraHeight && isFullScreen && drawLongTabs && newMap) {
				if(invOverlayInterfaceID != -1 && isFullScreen && newMap) {
...
//	entityMessage(npc);
 ...
 //		   return;
		...
		if(signlink.cache_dat != null)
		{
			...
			/*Preloaded sprites*/
			for(int i = 0; i < 4; i++)
				counter[i] = new Sprite("GameFrame/Orbs/XP/XP "+i);
			chatButtonH = new Sprite("Fullscreen/chathover");
			chatButtonC = new Sprite("Fullscreen/chatclicked");
			logIconH = new Sprite("GameFrame/Orbs/logiconh");
			logIconC = new Sprite("GameFrame/Orbs/logiconc");
			chatButtonHC = new Sprite("Fullscreen/chatclickedh");
			reportH = new Sprite("Fullscreen/reporthover");
			mapRing = new Sprite("Fullscreen/mapring");
			worldMapIcon = new Sprite("Fullscreen/mapicon");
			summonFill = new Sprite("GameFrame/Orbs/summonFill");
			summonFill2 = new Sprite("GameFrame/Orbs/summoned");
			...
			if (!newMap && !isFullScreen) {
				chatArea = new Sprite("Fullscreen/chatarea");
				tabArea = new Sprite("Fullscreen/tabarea");
				chatButtons = new Sprite("Fullscreen/chatbuttons");
				logIcon = new Sprite("Fullscreen/logout");
			}
			if (newMap && !isFullScreen) {
				chatArea = new Sprite("Fullscreen/chat");
				tabArea = new Sprite("Fullscreen/tab");
				chatButtons = new Sprite("Fullscreen/chatb");
				logIcon = new Sprite("Fullscreen/logout");
			}
			try
			{
				for(int i = 0; i <= 3; i++) {
					mapArea[i] = new Sprite("Fullscreen/map "+i+"");
				}
			}
			catch(Exception _ex) { }
			try
			{
				for(int i = 0; i <= 3; i++) {
					fullScreenSprites[i] = new Sprite("Fullscreen/tab "+i+"");
				}
			}
			catch(Exception _ex) { }
			/*Cache packed sprites*/
	...
	private void rightClickMapArea() {
		...
		int x = super.mouseX;
		int y = super.mouseY;
		if (super.mouseX >= 742 && super.mouseX <= 764
		...
		} else if (super.mouseX > 707 && super.mouseX < 762
				&& super.mouseY > 93 && super.mouseY < 124) {
...
*/
	...
	private void method146() {
		...
		if (isFullScreen) {
			...
			drawTabArea();
			drawChatArea();
			aRSImageProducer_1165.drawGraphics(0, bufferGraphics, 0);//Don't use super.graphics here, makes it update alot
			super.graphics.drawImage(buffer, 0, 0, getGameComponent());
		} else aRSImageProducer_1165.drawGraphics(isFullScreen ? 0 : 4, super.graphics, isFullScreen ? 0 : 4);
		xCameraPos = l;
		zCameraPos = i1;
		yCameraPos = j1;
		yCameraCurve = k1;
	...
	private void processXPCounterClick() {
        int x = super.mouseX;
        int y = super.mouseY;
        int x2 = isFullScreen ? 20+extraWidth : 0;
    	int y2 = isFullScreen ? -32 : 0;
        if (x >= 520+x2 && x <= 554+x2 && y >= 47+y2 && y <= 81+y2) {
	...
	Sprite[] counter = new Sprite[4];
    private void drawCounterOnScreen() {
        int x = 520;
        int y = 47;
        int x2 = isFullScreen ? 20+extraWidth : 0;
    	int y2 = isFullScreen ? -32 : 0;
        counter[2].drawSprite(x - 113+x2, y + 8+y2);
        int length = Integer.toString(xpCounter).length();
        int i = length == 1 ? 5 : ((length - 1) * 5);
        smallText.method382(0xFFFFFD, x - 98+x2, "XP:", y + 21+y2, true);
        if (xpCounter <= 214700000) {
        	smallText.method382(0xFFFFFD, x - 19+x2 - i, NumberFormat.getIntegerInstance().format(xpCounter), y + 21+y2, true);
        } else if (xpCounter >= 214700000) {
        	smallText.method382(0xFF0000, x - 18+x2 - i, "Lots!", y + 21+y2, true);
        }
        if (expAdded <= 0) {
            expAdded = 0;
        }
        if (expAdded > 0) {
            counter[3].drawSprite(x - 30+x2 - i - 45, y + 18 + xpAddedPos+y2);
            i = smallText.getTextWidth("" + NumberFormat.getIntegerInstance().format(expAdded) + "xp");
            smallText.method382(0xff9933, x - 2+x2 - i, "" + NumberFormat.getIntegerInstance().format(expAdded) + "xp", y + 21 + 18 + xpAddedPos+y2, true);
            xpAddedPos++;
            if (xpAddedPos >= 50) {
                xpAddedPos = 0;
                expAdded = 0;
    ...
    private void drawXPCounter() {
    		...
    		if(tabInterfaceIDs[12] != -1 && anInt1054 == 9)
    			if(loopCycle % 20 >= 12);
    				counter[0].drawSprite(0+x, 47+y);
    		if(xpIconHPos == 1) {
    			counter[1].drawSprite(0+x, 47+y);
    		}
    	}
	...
	public void drawHPOrb() {
		...
		int yPos = isFullScreen ? +32 : 0;
		if(isFullScreen) {
			emptyOrb = new Sprite("Fullscreen/oporb");
			} else {
			emptyOrb = new Sprite("Fullscreen/emptyorb");
		}
	...
	@SuppressWarnings("unused")
	private Sprite orbDrawn1;
	@SuppressWarnings("unused")
	private Sprite hoverOrb;
	@SuppressWarnings("unused")
	private Sprite orbDrawn2;
	public Sprite sumOrb;
	public Sprite summoningfill;
	public Sprite infinity;
	public Sprite summonFill;
	public Sprite summonFill2;
